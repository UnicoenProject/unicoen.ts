...
export class CPP14MapperBase implements CPP14Visitor<any> {
  ...
  public visitTerminal(node: TerminalNode) {
    ...
    if (token.type > 0) {
      ...
      for (; i < count; i++) {
        ...
        if (
          this._lastNode !== null &&
          this._stream.getTokens()[this._nextTokenIndex - 1].line == hiddenToken.line
        ) {
          if (this._lastNode.comments === null) {
            this._lastNode.comments = [];
          }
      ...
      for (
        i = count + 1;
        i < count2 &&
        this._stream.getTokens()[i].channel == Token.HIDDEN_CHANNEL &&
        this._stream.getTokens()[count].line == this._stream.getTokens()[i].line;
        i++
      ) {
